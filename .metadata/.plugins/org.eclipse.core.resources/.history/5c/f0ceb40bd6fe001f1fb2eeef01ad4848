...
unsigned char modeTable[] = { 0x17, 0x37, 0x17, 0x17 };
unsigned char driveStrengthTable[] = { 0x06, 0x06, 0x06, 0x06 };
...
void IssueNandReq(unsigned int chNo, unsigned int wayNo)
{
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_SET_FEATURE)
	{
		...
		V2FSetFeaturesSync(&chCtlReg[chNo], wayNo, modeTable[chNo], driveStrengthTable[chNo], 0, TEMPORARY_PAY_LOAD_ADDR);
	}
	else
