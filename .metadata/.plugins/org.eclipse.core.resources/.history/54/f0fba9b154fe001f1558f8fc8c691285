...
#define ZONE_MANAGEMENT_RECV_ADDR	    (NAMESPACE_IDENTIFICATION_ADDR + NAMESPACE_IDENTIFICATION_SIZE)//4KB
#define ZONE_MANAGEMENT_RECV_SIZE   	4096

#define STORAGE_CAPACITY_L				NUMBER_OF_SECTORS_ON_DEVICE//(4*8*2*4096*128*4) * 4KB = 256GB//not used
...
#define IO_NVM_ZONE_MGMT_SEND		    					0x79
#define IO_NVM_ZONE_MGMT_RECV 			    				0x7a
#define IO_NVM_ZONE_APPEND					        		0x7d
...
/* Identify ZNS Controller Data Structure */
typedef struct _ADMIN_IDENTIFY_ZNS_CONTROLLER
{
    unsigned char     zasl;
    unsigned char     rsvd1[4095];
} ADMIN_IDENTIFY_ZNS_CONTROLLER;

typedef struct NvmeLBAFE {
    unsigned long long   zsze;
    unsigned char        zdes;
    unsigned char        rsvd9[7];
} NvmeLBAFE;

/* Identify ZNS Namespace Data Structure */
typedef struct _ADMIN_IDENTIFY_ZNS_NAMESPACE
{
    unsigned short    zoc;
    unsigned short    ozcs;
    unsigned int      mar;
    ...
    unsigned int      rrl;
    unsigned int      frl;
    unsigned char     rsvd12[24];
    unsigned int      numzrwa;
    unsigned short    zrwafg;
    unsigned short    zrwas;
    unsigned char     zrwacap;
    unsigned char     rsvd53[2763];
    NvmeLBAFE         lbafe[16];
    unsigned char     rsvd3072[768];
    unsigned char     vs[256];
} ADMIN_IDENTIFY_ZNS_NAMESPACE;
