...
{
	ADMIN_IDENTIFY_ZNS_NAMESPACE *identifyNS;
	identifyNS = (ADMIN_IDENTIFY_ZNS_NAMESPACE *)pBuffer;

    memset(identifyNS, 0, sizeof(ADMIN_IDENTIFY_ZNS_NAMESPACE));

    /* MAR/MOR are zeroes-based, FFFFFFFFFh means no limit */
    identifyNS->mar = ns->id_ns_zoned->mar;
    identifyNS->mor = ns->id_ns_zoned->mor;
    identifyNS->zoc = ns->id_ns_zoned->zoc;
    identifyNS->ozcs = ns->id_ns_zoned->ozcs;
    identifyNS->lbafe[0].zsze = ns->id_ns_zoned->lbafe[0].zsze;
    identifyNS->lbafe[0].zdes = ns->id_ns_zoned->lbafe[0].zdes;
}

void zns_controller_identification(unsigned int pBuffer)
{
	ADMIN_IDENTIFY_ZNS_CONTROLLER *identifyCNTL;
	identifyCNTL = (ADMIN_IDENTIFY_ZNS_CONTROLLER *)pBuffer;

	memset(identifyCNTL, 0, sizeof(ADMIN_IDENTIFY_ZNS_CONTROLLER));

    identifyCNTL->zasl = ns->params.zasl;
}

void nvme_identify_cmd_set(unsigned int pBuffer)
{
    unsigned char *list = (unsigned char *)pBuffer;
