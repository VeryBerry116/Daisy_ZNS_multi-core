...
NVME_M2_REQUEST_QUEUE nvmeM2ReqQ[NUM_NVME_M2];

unsigned int notCompletedNandReqCnt;
unsigned int blockedReqCnt;
unsigned int notCompletedNvmeM2ReqCnt;
...
void SelectiveGetFromNvmeDmaReqQ(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.forceUnitAccess == REQ_OPT_FORCE_UNIT_ACCESS_ON)
		CompleteChildDmaReqOfNvmeIoCmd(reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
...
void GetFromNandReqQ(unsigned int chNo, unsigned int wayNo, unsigned int reqStatus, unsigned int reqCode)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.forceUnitAccess == REQ_OPT_FORCE_UNIT_ACCESS_ON)
		CompleteChildNandReqOfNvmeIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqStatus, reqCode);
...
void PutToNvmeM2ReqQ(unsigned int reqSlotTag)
{
	...
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = nvmeM2ReqQ[chNo].tailReq;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[nvmeM2ReqQ[chNo].tailReq].nextReq = reqSlotTag;
		nvmeM2ReqQ[chNo].tailReq = reqSlotTag;
	}
	else
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		nvmeM2ReqQ[chNo].headReq = reqSlotTag;
		nvmeM2ReqQ[chNo].tailReq = reqSlotTag;
	}

	reqPoolPtr->reqPool[reqSlotTag].reqQueueType = REQ_QUEUE_TYPE_NVME_M2;
	nvmeM2ReqQ[chNo].reqCnt++;
	notCompletedNvmeM2ReqCnt++;
}
...
void GetFromNvmeM2ReqQ(unsigned int chNo)
{
	...
	reqSlotTag = nvmeM2ReqQ[chNo].headReq;
	if(reqSlotTag == REQ_SLOT_TAG_NONE)
		assert(!"[WARNING] there is no request in Nand-req-queue[WARNING]");

	if(reqPoolPtr->reqPool[reqSlotTag].nextReq != REQ_SLOT_TAG_NONE)
	{
		nvmeM2ReqQ[chNo].headReq = reqPoolPtr->reqPool[reqSlotTag].nextReq;
		reqPoolPtr->reqPool[reqPoolPtr->reqPool[reqSlotTag].nextReq].prevReq = REQ_SLOT_TAG_NONE;
	}
	else
	{
		nvmeM2ReqQ[chNo].headReq = REQ_SLOT_TAG_NONE;
		nvmeM2ReqQ[chNo].tailReq = REQ_SLOT_TAG_NONE;
	}
