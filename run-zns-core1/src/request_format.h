...
#define REQ_QUEUE_TYPE_NVME_M2						0x7
...
typedef struct _REQ_OPTION{
	unsigned int dataBufFormat : 2;  //data buffer entry, temp data buffer entry, device addr
...
typedef struct _SSD_REQ_FORMAT
{
	...
	NAND_INFO nandInfo;
	unsigned int MPTR[2];//host addr//read//device->host

	unsigned int prevReq : 16;	//maximum outstanding request 64K;
	unsigned int nextReq : 16;
	unsigned int prevBlockingReq : 16;	//maximum outstanding request 64K;
