...
void InitNvmeIoCmdCompleteTable()
{
	...
	for(cmdSlotTag=0; cmdSlotTag < MAX_CMD_SLOT_TAG; cmdSlotTag++)
	{
		...
		nvmeIoCmdCompleteTablePtr->nvmeIoCmdCompleteEntry[cmdSlotTag].w_ptr = 0;
	}
}
...
void AssignWPtrNvmeIoCmd(unsigned int cmdSlotTag, unsigned long long w_ptr)
{
    nvmeIoCmdCompleteTablePtr->nvmeIoCmdCompleteEntry[cmdSlotTag].w_ptr = w_ptr;
}
...
void CompleteChildNandReqOfNvmeIoCmd(unsigned int cmdSlotTag, unsigned int reqStatus, unsigned int reqCode)
{
	...
	if(nvmeIoCmdCompleteTablePtr->nvmeIoCmdCompleteEntry[cmdSlotTag].notCompletedChildNandReqCount > 0)
	{
		if(--nvmeIoCmdCompleteTablePtr->nvmeIoCmdCompleteEntry[cmdSlotTag].notCompletedChildNandReqCount == 0)
		{
			...
			w_ptr = nvmeIoCmdCompleteTablePtr->nvmeIoCmdCompleteEntry[cmdSlotTag].w_ptr;
			...
			set_auto_nvme_cpl(cmdSlotTag, w_ptr, nvmeCPL.statusFieldWord);
